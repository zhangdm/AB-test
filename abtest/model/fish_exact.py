...
def fisher_test(Data,batcha,batchb,alpha,input_power):
    data = pd.DataFrame(Data, dtype='float')
    alpha,input_power = float(alpha),float(input_power)

    batcha,batchb = int(batcha),int(batchb)
    ...
    pvalue = fisher_exact([[dfa.shape[0] - N_a_0,N_a_0],[dfb.shape[0] - N_b_0,N_b_0]])[1]
